...
GET  /api/permissions/all                    // Get all permissions (Super Admin only)
GET  /api/permissions/role/:role             // Get role permissions (Super Admin only)
GET  /api/permissions/category/:category     // Get permissions by category (Super Admin only)
POST /api/permissions/grant                  // Grant permission (Super Admin only)
POST /api/permissions/revoke                 // Revoke permission (Super Admin only)
GET  /api/permissions/audit-log              // Get audit log (Super Admin only)
GET  /api/permissions/check/:permissionKey   // Check current user permission
POST /api/permissions/reset-defaults         // Reset to defaults (Super Admin only)
...
- Prototype Plan: `docs/coi-system/Prototype_Plan.md`
- Risk Analysis: `docs/coi-system/Risk_Analysis_and_Failure_Points.md`
- User Journeys: `docs/coi-system/User_Journeys_End_to_End.md`
- Dynamic Fields: `docs/coi-system/COI_Template_Fields_Extraction.md`
- Fuzzy Matching: `docs/coi-system/Fuzzy_Matching_Implementation_Location.md`
...
**Context:** PRMS Client Master often has parent company labelled **"TBD"**. Users may capture or update parent in COI or in PRMS; both systems should stay aligned. **Rule:** If data is first updated in COI, pushing to PRMS must go through **PRMS Admin approval** (parent company update request workflow).

**Features Added**:
1. **Parent company update request** table and API: COI-originated parent updates (when PRMS has TBD/empty) create a request; PRMS Admin approves or rejects; on approve, PRMS is updated (mock in prototype).
2. **Read from PRMS**: When loading an existing BDO client, `parent_company` is included; value **"TBD"** or empty is normalized to null so the user can enter parent in COI.
3. **Save in COI**: When user sets/updates parent on an existing client and PRMS has TBD/empty, backend creates a parent company update request (Pending). No direct PRMS write.
4. **PRMS Admin UI**: Admin Dashboard → "Parent Company Updates" tab → list, approve, reject (with optional note).

| Aspect | Prototype | Production |
|--------|-----------|------------|
| **PRMS read (existing client)** | Local `clients` table; `parent_company` returned; TBD/empty → null for pre-fill | Call PRMS Client Master (or sync); same TBD/empty contract |
| **PRMS write (parent update)** | On approve: update local `clients.parent_company` (mock) | On approve: call PRMS API to update client parent_company; then mark request Completed |
| **Queue & approval** | Local `parent_company_update_requests`; same approval UI | Same approval flow; production replaces mock write with real PRMS API |
| **Sync PRMS → COI** | N/A (single source in prototype) | When PRMS is updated (by PRMS users or after approved COI update), COI notified (webhook/poll/event); update `coi_requests`/`clients` |

**Client vs Prospect:**
- **Existing client (BDO list):** PRMS integration required — read client (and parent) from PRMS when loading; parent updates from COI → parent company update request → PRMS Admin approval → write to PRMS.
- **Prospect / New prospect:** Not in PRMS until conversion; PRMS only at conversion (client creation) with existing PRMS Admin validation; parent from COI request included in client creation.

**Production Notes**:
- Document in handoff: GET client (by id/code) including `parent_company`; contract for TBD/empty. PATCH/PUT client `parent_company` used only after PRMS admin approval from COI.
- Who performs the write: COI backend with service account vs PRMS accepting an approved queue — document chosen approach.
- Code: `parent_company_update_requests` table, `parentCompanyUpdateController.js`, `parentCompanyUpdate.routes.js`, COI create/submit in `coiController.js`, Admin tab `ParentCompanyUpdateRequestsPanel.vue`. See `coi-prototype/PRMS_INTEGRATION_CURRENT_STATE.md` (Parent Company Bidirectional Sync section).
...
1. **Permission Management System**:
   - Dynamic permission catalog (24 permissions across 4 categories)
   - Role-permission mapping with grant/revoke functionality
   - Permission audit trail for all changes
   - Super Admin has all permissions (cannot be modified)
   - Fail-closed security (default deny if permission not found)
...
3. **Backend Service** (`permissionService.js`):
   - `checkPermission(userRole, permissionKey)` - Check if role has permission
   - `getRolePermissions(role)` - Get all permissions for a role
   - `grantPermission(role, permissionKey, grantedBy)` - Grant permission
   - `revokePermission(role, permissionKey, revokedBy, reason)` - Revoke permission
   - `getAllPermissions()` - Get all available permissions
   - `getPermissionAuditLog(filters)` - Get audit trail

4. **Backend Middleware** (`auth.js`):
   - `requirePermission(permissionKey)` - Permission-based route protection
...
5. **Backend API Endpoints** (`/api/permissions`):
   - `GET /api/permissions/all` - Get all permissions (Super Admin only)
   - `GET /api/permissions/role/:role` - Get role permissions (Super Admin only)
   - `GET /api/permissions/category/:category` - Get permissions by category (Super Admin only)
   - `POST /api/permissions/grant` - Grant permission (Super Admin only)
...
6. **Frontend Composable** (`usePermissions.ts`):
   - `hasPermission(permissionKey)` - Check single permission
   - `hasAnyPermission(permissionKeys[])` - Check any of multiple permissions
   - `hasAllPermissions(permissionKeys[])` - Check all of multiple permissions
   - `canAccess(route)` - Check route access
   - Permission caching (5-minute cache)
   - Auto-loads permissions for current user role

7. **Frontend UI** (`PermissionConfig.vue`):
   - Permission matrix view (Roles × Permissions)
   - Category and role filtering
   - Toggle permissions per role
   - Audit log display
   - Reset to defaults functionality
   - Route: `/coi/admin/permission-config` (Super Admin only)

8. **Default Permissions Seeded** (24 permissions):
   - Configuration: email.config.*, sla.config.*, priority.config.*
   - User Management: users.create, users.edit, users.disable, users.view
   - System: system.edition.switch, system.audit.view, system.config.view, permissions.manage
   - Requests: requests.create, requests.approve.*, requests.generate.code, requests.execute, requests.view.all

**Database Schema**:
- Migration: `database/migrations/20260126_permissions.sql`
- Tables auto-created in `initDatabase()` via `createPermissionTables()`
- Default permissions seeded via `seedDefaultPermissions()`

**Production Notes**:
- **SQL Server Migration**: Same schema structure, convert SQLite syntax to SQL Server
- **SSO Integration**: Permission checks should integrate with SSO/Active Directory user groups
- **HRMS User Groups**: Map HRMS user groups to COI permissions (see HRMS Integration section)
- **Stored Procedures**: Consider creating stored procedures for:
  - Bulk permission operations
  - Permission inheritance rules
  - Permission validation
- **Performance**: Indexes are created for all lookup patterns (role, permission_key, category, date)
- **Audit Trail**: All permission changes are logged with user ID, timestamp, and optional reason
- **Super Admin Protection**: Super Admin permissions cannot be revoked (enforced in code)
- **Fail-Closed Security**: Default deny if permission not found (security best practice)
- **Component Migration**: Gradually migrate components from hardcoded role checks to `usePermissions()` composable

**Related Files**:
- Backend: `backend/src/services/permissionService.js`, `backend/src/routes/permission.routes.js`, `backend/src/middleware/auth.js`
- Frontend: `frontend/src/composables/usePermissions.ts`, `frontend/src/views/PermissionConfig.vue`
- Database: `database/migrations/20260126_permissions.sql`
- Documentation: `coi-prototype/PERMISSIONS_REFERENCE.md`, `coi-prototype/PERMISSION_SYSTEM_VERIFICATION_REPORT.md`
...
| 2026-01-26 | `permissions`, `role_permissions`, `permission_audit_log` |
